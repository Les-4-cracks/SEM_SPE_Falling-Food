...
//charge toutes les images de sprites
    ...
    if (!jouerimg1.loadFromFile("image/menu/menuJouer1.png"))
        printf("Echec chargment\n");
    if (!optionsimg1.loadFromFile("image/menu/menuCredits1.png"))
        printf("Echec chargment\n");
    if (!reglesimg1.loadFromFile("image/menu/menuRègles1.png"))
        printf("Echec chargment\n");
    if (!jouerimg2.loadFromFile("image/menu/menuJouer2.png"))
        printf("Echec chargment\n");
    if (!optionsimg2.loadFromFile("image/menu/menuCredits2.png"))
        printf("Echec chargment\n");
...
void credits(RenderWindow &fenetre)
{
    bool boutonretour;
    Sprite spfond,spretour;
    Texture fondimg,retourimg1,retourimg2;

    if (!fondimg.loadFromFile("image/menu/credits.png"))
        printf("Echec chargment\n");
            ...
            while (fenetre.pollEvent(event))
            {
                ...
                if (event.type == Event::MouseButtonPressed)
                {
                    if (event.mouseButton.button == Mouse::Left )
                    {
                        ...
                        if (boutonretour)
                        {
                            creditsEtat=1;//permet de quitter la boucle fonction credits() et donc de revenir dans la fonction menu()
                        }
